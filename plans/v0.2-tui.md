...
- Dependency detail modal has a "Dependency" tab:
  - Rename alias (changes depID; migrates depID-keyed data in `values.instance.yaml`, `values.dep-set.<depID>--*.yaml`, and `.helmdex/depmeta/<instance>/<depID>.yaml`)
  - Delete dependency (cleans up the same depID-keyed data)
